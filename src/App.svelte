<script>
	...
	const toggleLoader = () => { document.getElementById(`modal`).classList.toggle(`hidden`); }

	// Just a little thing to update the modal text when needed.
	const updateLoader = (message) => { document.getElementById(`loader-message`).innerText = message; }
	...
	const fetchTransactions = async (address) => {
		...
		while (true) {
			...
			updateLoader(`Fetching transaction IDs ${startingValue} - ${endingValue}`)
			let accountHistoryBlock = await cnx.getConfirmedSignaturesForAddress2(
				new PublicKey(address),
				{ before: before }
			)
			if (accountHistoryBlock.length === 0) { break }
			accountHistory = accountHistory.concat(accountHistoryBlock.map(i => i.signature))
			before = accountHistoryBlock[accountHistoryBlock.length - 1]['signature']
			i += 1
			await sleep(1000)
		}
		updateLoader(`Finished. Fetched ${accountHistory.length} transaction IDs.`)
		await sleep(1000)
	...
	const fetchTransactionsInfo = async (signatures) => {
		...
		for (const signatureChunk of signatureChunks) {
			updateLoader(`Fetching transaction chunk ${j}/${signatureChunks.length}`)
...
<main>
	...
	<div class="container">
		...
		<input id="address" class="address" type="text" required placeholder="Your Solana account address..." autocorrect="off" autocapitalize="off" spellcheck="false">
		<div class="btn" on:click={sendToExtract}>
			extract me daddy
		</div>
	</div>
</main>
